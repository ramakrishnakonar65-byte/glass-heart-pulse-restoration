...
export const Card = forwardRef<HTMLDivElement, { customClass?: string; [key: string]: any }>(({ customClass, ...rest }, ref) => (
...
interface CardSwapProps {
  ...
  delay?: number; pauseOnHover?: boolean; onCardClick?: (i: number) => void;
  skewAmount?: number; easing?: 'elastic' | 'smooth'; children: ReactNode;
}
  ...
  const rendered = childArr.map((child, i) =>
    isValidElement(child)
      ? cloneElement(child as ReactElement<any>, {
          key: i, ref: refs[i],
          style: { width, height, ...(((child as ReactElement<any>).props as any).style ?? {}) },
          onClick: (e: React.MouseEvent) => { ((child as ReactElement<any>).props as any).onClick?.(e); onCardClick?.(i); }
        })
      : child
  );
